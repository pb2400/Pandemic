...
void pandemic::values_checking() {
  ...
  if (initial_state.n_ <= 0) {
    throw std::runtime_error{
        "there must be at least one person in the population"};
  }

  // checks the ratio between beta and gamma
  // has to be greater than one to let the pandemic start
  if (beta_ / gamma_ <= 1) {
    throw std::runtime_error{"with these starting values of beta and gamma the "
                             "pandemic won't start"};
  }
...
// this method's task is to create a vector
...
std::vector<population_state> pandemic::population_check(int d) {
  ...
  if (d <= 0) {
    throw std::runtime_error{"either the pandemic has been set on day 0 or the "
                             "number of day is not valid"};
  }
