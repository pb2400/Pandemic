...
int neighbours_infected(world const& population_, int r, int c)
{
    ...
    //the fact that the person analyzed is counted too doesnt'influence the result 
    //because we enter the loop only if the person is susceptible so the counter
    //will not increase the value by one
