...
void print(world const &population) {
  ...
  // person that day is analyzed and the function prints and empty char if the
  // person is s_ a '*' if the person is infected and a '°' if the person is
  // removed once completed a row the first if allows to switch row, and restart
  // the cycle
  for (int r = 0; r != n; ++r) {
    std::cout << '|';
    for (int c = 0; c != n; ++c) {
...
int main() {
  ...
  std::cin >> gamma;
  std::cout
      << "choose how many stages of the pandemic simulation you want to see "
      << '\n';
  std::cin >> stages;
  std::cout << "choose the size of graph's side (20 < size < 50) " << '\n';
  std::cin >> side;
  ...
  try {
    ...
    // this for is used to assing to 1/25 of the population at day 0 the value
    // of people infected. the people are choosen randomly through the generator
    // created before
    for (int i = 0; i != side * side / 25; ++i) {
      ...
      // this for is needed to regenerate the coordinates of the person we want
      // to establish as infected
