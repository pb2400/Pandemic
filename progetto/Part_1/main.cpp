...
void print(std::vector<population_state> const &pop_daily_) {
  ...
  for (auto &dates_ : pop_daily_) {
    std::cout << "| " << std::setw(9) << dates_.susceptibles_ << " | "
              << std::setw(9) << dates_.infected_ << " | " << std::setw(9)
              << dates_.removed_ << " |\n";
  }
  // building the bottom of the table
  std::cout << "+-----------+-----------+-----------+\n";
}

int main() {
  // using int n_, s_, i_, r_ to represent the values of susceptibles, infected
  // and removed using g_, b_ and d_ to represent the values of gamma, beta and
  // days
  int n_;
  int s_;
  int i_;
  ...
  double g_;
  double b_;
  std::cout << "insert the starting values of population: peoples, "
               "susceptibles, infected and removed "
            << '\n';
  std::cin >> n_ >> s_ >> i_ >> r_;
